...
flags.DEFINE_integer('k1', default=500, help='k1')
flags.DEFINE_integer('k2_epoch', default=15, help='k2')
    ...
    while True:
        ...
        for j in range(FLAGS.batchs_per_epoch):
            ...
            if steps % FLAGS.k1 == 0:
                student = Model()
                model.eval()
                new_opt = optim.Adam(student.parameters(), lr=5e-4)
                mean_repr = teach.get_mean_repr(dataset, teacher=model)
                student.train()
                for i in range(FLAGS.k2_epoch):
                    trn_lb = teach.train_loop(dataset, teacher=model, student=student,
                                              opt=new_opt, mean_repr=mean_repr)
